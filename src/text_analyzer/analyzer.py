...
class TextAnalyzer(Preprocessor):
    ...
    def __init__(self, text: str) -> None:
        ...
        self.results: list[SentenceResult] = [
            SentenceResult(id=i, text=sent) for i, sent in enumerate(self.sentences, start=1)
        ]
        logger.info("TextAnalyzer is ready")

    def find_imperatives(
        self, pos: str = "VERB", tag_name: str = "Mood", tag_value: str = "Imp"
    ) -> list[SentenceResult]:
        """
        Extract imperatives.
    ...
    def load_interrogative_words(self) -> set[str]:
        ...
        try:
            ...
            with open(INTERROGATIVE_WORDS_PATH, encoding="utf-8") as f:
    ...
    def detect_questions(self) -> list[SentenceResult]:
        ...
        for result in self.results:
            ...
            result.is_question = {"general question": is_general, "specific question": is_specific}
